...
REGISTER_OP("CalIvt")
    ...
    .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
        ::tensorflow::shape_inference::ShapeHandle trisshape; // batch_size * maxntris * 3 * 3
        c->WithRank(c->input(0), 4, &trisshape);
        ::tensorflow::shape_inference::ShapeHandle pointsshape; // batch_size * n * 3
        c->WithRank(c->input(2), 3, &pointsshape);
        // batch_size * npoints
        ::tensorflow::shape_inference::ShapeHandle ivtsshape = c->MakeShape({c->Dim(pointsshape, 0), c->Dim(pointsshape, 1), c->Dim(pointsshape, 2)});
        c->set_output(0, ivtsshape);
        return Status::OK();
    });
REGISTER_OP("CalIvtGrad")
    .Input("verts: float32")
    .Input("tris: int32")
    .Input("r1: float32")
    .Input("r2: float32")
...
class MeshSamplingGradGpuOp : public OpKernel{
    public:
        ...
        void Compute(OpKernelContext * context)override{
            ...
            OP_REQUIRES_OK(context,context->allocate_output(0,TensorShape{b,maxnverts,n_c},&grad_feats_tensor));
