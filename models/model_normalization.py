    ...
    if FLAGS.img_feat_onestream:
        ...
        if not FLAGS.multi_view:
            ...
            with tf.compat.v1.variable_scope("sdfprediction") as scope:
                pred_ivts_value_global = ivtnet.get_ivt_basic(input_pnts_rot, ref_feats_embedding_cnn, is_training, batch_size, FLAGS.num_pnts, bn, bn_decay, wd=wd)

            with tf.compat.v1.variable_scope("sdfprediction_imgfeat") as scope:
                pred_ivts_value_local = ivtnet.get_ivt_basic_imgfeat_twostream(input_pnts_rot, point_img_feat, is_training, batch_size, FLAGS.num_pnts, bn, bn_decay, wd=wd)

            pred_ivt = pred_ivts_value_global + pred_ivts_value_local
            end_points["pred_ivts_value_global"] = pred_ivts_value_global
            end_points["pred_ivts_value_local"] = pred_ivts_value_local
    else:
        if not FLAGS.multi_view:
            with tf.compat.v1.variable_scope("sdfprediction") as scope:
