...
void create_mesh(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud,int& surface_mode,int& normal_method,pcl::PolygonMesh& triangles){
     ...
     if(gp3_mode){
       ...
       int setMU = 40;
       int maxiNearestNeighbors = 1000;
       bool normalConsistency = false;
       printf("searchK %d, search_radius %d, setMU %d, maxiNearestNeighbors %d", searchK, search_radius, setMU, maxiNearestNeighbors);
       ...
       std::cout << "[OK]" << std::endl;
        ...
        bool manifold= false;
        int solverDivide=8;
        printf("setKsearch %d, depth %d, pointWeight %f, samplePNode %f, scale %f, isoDivide %d", setKsearch, depth, pointWeight, samplePNode, scale, isoDivide);
