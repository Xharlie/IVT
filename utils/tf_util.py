...
def _variable_with_weight_decay(name, shape, stddev, wd, use_xavier=True):
    ...
    if wd is not None:
        ...
        tf.compat.v1.add_to_collection('regularizer', weight_decay)
    return var
...
def ResidueBlock(inputs, is_training, in_channels, out_channels, scope, add_kernel, stride, activation_fn=tf.nn.relu, residue_ratio=0.5, use_BatchStatistics=False, bn=True, bn_decay=None, wd=None):
    with tf.compat.v1.variable_scope(scope) as sc:
        residue_ratio = math.sqrt(residue_ratio)
        shortcut_ratio = math.sqrt(1 - residue_ratio)
        residue = conv2d(inputs, out_channels, [stride + add_kernel * 2, stride + add_kernel * 2], padding='SAME', stride=[stride, stride], activation_fn=activation_fn, bn=bn, bn_decay=bn_decay, is_training=is_training, weight_decay=wd, scope='conv_residue')
        shortcut = inputs
        if stride == 2:
            shortcut = avg_pool2d(shortcut, [stride, stride], scope, stride=[stride, stride], padding='SAME')
        if in_channels != out_channels:
            shortcut = conv2d(shortcut, out_channels, [1,1], padding='VALID', stride=[1, 1], activation_fn=activation_fn, bn=bn, bn_decay=bn_decay, is_training=is_training, weight_decay=wd, scope='conv_shortcut')
        return shortcut * shortcut_ratio + residue * residue_ratio